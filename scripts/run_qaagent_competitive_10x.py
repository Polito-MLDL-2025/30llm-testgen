...
def list_log_dirs(logs_root: Path) -> set[Path]:
    if not logs_root.exists():
        return set()
    prefixes = ("multi_agent-", "QAagent-", "QAagent_competitive-")
    return {
        path.resolve()
...
def _run_agent_main(argv: list[str], output_queue: multiprocessing.Queue) -> None:
    import traceback
    from llm30.pipeline.QAagent import QAagent_competitive
    ...
    try:
        exit_code = QAagent_competitive.main(argv)
    ...
    while True:
        ...
        while True:
            ...
            if not process.is_alive():
                ...
                if exit_code == 0:
                    return
                raise RuntimeError(f"QAagent competitive exited with code {exit_code}")
...
def main(argv=None) -> int:
    ...
    if args.predefine_name:
        ...
        base_name = f"{timestamp}_qaagen_competitive_{dataset_name}_{model_name}"
